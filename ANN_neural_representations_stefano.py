...
n_wei=10
wei_vec=np.ones((n_wei,2))
wei_vec[:,0]=np.linspace(1,5,n_wei)
print (wei_vec)
...
weights_all=nan*np.zeros((len(wei_vec),n_files,n_hidden+2))
for hh in range(n_wei):
    wei_ctx=wei_vec[hh]
    print (wei_ctx)

    for gg in range(n_files):
        ...
        weights_all[hh,gg,0:-1]=weights
        weights_all[hh,gg,-1]=bias

weights_all_m=np.mean(abs(weights_all),axis=1)
weights_all_sem=sem(abs(weights_all),axis=1)

fig=plt.figure(figsize=(3,2.5))
ax=fig.add_subplot(111)
miscellaneous.adjust_spines(ax,['left','bottom'])
for i in range(n_hidden+2):
    if i==(n_hidden+1): #general bias
        col='black'
        lab='w_0'
        sem=True
    elif i==(n_hidden): #context bias modulation
        col='green'
        lab='w_ctx'
        sem=True
    elif i==(n_hidden-1):
        ...
        lab=None
        sem=False
    ax.plot(np.arange(n_wei),weights_all_m[:,i],color=col,label=lab)
    if sem:
        ax.fill_between(np.arange(n_wei),weights_all_m[:,i]-weights_all_sem[:,i],weights_all_m[:,i]+weights_all_sem[:,i],color=col,alpha=0.5)
ax.set_xlabel('Strength Reward Asymmetry')
ax.set_ylabel('Mean Absolute Weights')
plt.legend(loc='best')
fig.savefig('/home/ramon/Dropbox/Proyectos_Postdoc/Esteki_Kiani/plots/weights_asymmetry_stefano.pdf',dpi=500,bbox_inches='tight')
