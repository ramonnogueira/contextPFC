...
beha_te_unte_all=nan*np.zeros((2,2,9,t_back+t_forw))
...
for k in range(len(monkeys)):
    ...
    for hh in range(len(files_groups)):
        ...
        fit_beha_all[0,0,uu,(t_back+1):]=aa00
        fit_beha_all[0,0,uu,0:t_back]=np.nanmean(beha_te_unte[0,0,hh,0:t_back])
        fit_beha_all[0,1,uu,(t_back+1):]=aa01
        fit_beha_all[0,1,uu,0:t_back]=np.nanmean(beha_te_unte[0,1,hh,0:t_back])
        fit_beha_all[1,0,uu,(t_back+1):]=aa10
        fit_beha_all[1,0,uu,0:t_back]=np.nanmean(beha_te_unte[1,0,hh,0:t_back])
        fit_beha_all[1,1,uu,(t_back+1):]=aa11
        fit_beha_all[1,1,uu,0:t_back]=np.nanmean(beha_te_unte[1,1,hh,0:t_back])
        y0_beha_all[0,0,uu]=aa00[0]
        y0_beha_all[0,1,uu]=aa01[0]
        y0_beha_all[1,0,uu]=aa10[0]
        y0_beha_all[1,1,uu]=aa11[0]
    ...
    plt.legend(loc='best')
    fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/beha_rt_inference2_%s_%s_lowR_stim.pdf'%(monkeys[k],delta_type),dpi=500,bbox_inches='tight')
    ...
    ax.set_ylabel('Normalized RT')
    plt.legend(loc='best')
    fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/beha_rt_inference2_%s_%s_highR_stim.pdf'%(monkeys[k],delta_type),dpi=500,bbox_inches='tight')
...
beha_fit_m=np.nanmean(fit_beha_all,axis=2)
beha_fit_sem=sem(fit_beha_all,axis=2,nan_policy='omit')
if delta_type=='raw':
    delta_beha=(beha_te_unte_all[:,:,:,t_back+1]-np.nanmean(beha_te_unte_all[:,:,:,0:t_back],axis=3))
if delta_type=='fit':
    delta_beha=(y0_beha_all-np.nanmean(beha_te_unte_all[:,:,:,0:t_back],axis=3))
delta_beha_m=np.nanmean(delta_beha,axis=2)
delta_beha_sem=sem(delta_beha,axis=2,nan_policy='omit')
