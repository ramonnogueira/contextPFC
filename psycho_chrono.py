...
params_psy_all=nan*np.zeros((20,2,3))
params_rt_all=nan*np.zeros((20,6,3))
...
for k in range(len(monkeys)):
    ...
    for kk in range(len(files)):
        ...
        rew_pairs=beha['rew_pairs']
        mrew=np.mean(0.5*(rew_pairs[:,0]+rew_pairs[:,1]))

        rt_con=rt[((stimulus==0)&(context==0))|((stimulus==1)&(context==1))]
        rt_inc=rt[((stimulus==0)&(context==1))|((stimulus==1)&(context==0))]
        #if (k==0) and (kk==0):
        if kk==0:
            rt_con_all=rt_con.copy()
            rt_inc_all=rt_inc.copy()
        else:
            rt_con_all=np.hstack((rt_con_all,rt_con))
            rt_inc_all=np.hstack((rt_inc_all,rt_inc))
        print (len(rt_con),len(rt_inc))
        print (len(rt_con_all),len(rt_inc_all))      

        # print (len(reward),len(ind_ch))
        # t_ctx.append(len(reward)/len(ind_ch))
        # print (t_ctx)

        # ctx0_col=nan*np.zeros(len(rew_pairs))
        # ctx1_col=nan*np.zeros(len(rew_pairs))
        # ctx0_col[context==0]=mrew
        # ctx1_col[context==1]=mrew
        # plt.close('all')
        # plt.plot(rew_pairs[:,1],color='green')
        # plt.plot(rew_pairs[:,0],color='blue')
        # plt.plot(ctx0_col,color='green',linestyle='--',lw=3)
        # plt.plot(ctx1_col,color='blue',linestyle='--',lw=3)
        # plt.title('Session %i'%(kk+1))
        # plt.ylabel('Open valve time')
        # plt.xlabel('Trial')
        # if monkeys[k]=='Niels':
        #     plt.ylim([250,550])
        # if monkeys[k]=='Galileo':
        #     plt.ylim([200,400])
        # plt.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/%s_valve_open_session_%i.png'%(monkeys[k],kk+1))        
    ...
    print (np.nanmean(rt_con_all),sem(rt_con_all,nan_policy='omit'))
    print (np.nanmean(rt_inc_all),sem(rt_inc_all,nan_policy='omit'))
...
#     fig.savefig('/home/ramon/Dropbox/Proyectos_Postdoc/Esteki_Kiani/plots/psychometric_monkey_%s_def2.pdf'%monkeys[k],dpi=500,bbox_inches='tight')
...
#     bax.plot(xx_fit,fit_chrono_m[:,0],color='black',alpha=0.5)
#     bax.fill_between(xx_fit,fit_chrono_m[:,0]-fit_chrono_sem[:,0],fit_chrono_m[:,0]+fit_chrono_sem[:,0],color='black',alpha=0.5)
#     bax.plot(xx_fit,fit_chrono_m[:,1],color='green',linewidth=1,alpha=0.5)
#     bax.fill_between(xx_fit,fit_chrono_m[:,1]-fit_chrono_sem[:,1],fit_chrono_m[:,1]+fit_chrono_sem[:,1],color='green',alpha=0.5)
#     bax.plot(xx_fit,fit_chrono_m[:,2],color='blue',linewidth=1,alpha=0.5)
#     bax.fill_between(xx_fit,fit_chrono_m[:,2]-fit_chrono_sem[:,2],fit_chrono_m[:,2]+fit_chrono_sem[:,2],color='blue',alpha=0.5)
#     bax.scatter(xx_coh,chrono_m[:,0],color='black',s=3)
#     bax.scatter(xx_coh,chrono_m[:,1],color='green',s=3)#,alpha=0.5)
#     bax.scatter(xx_coh,chrono_m[:,2],color='blue',s=3)#,alpha=0.5)
#     bax.set_ylabel('Reaction Time (ms)')
#     bax.set_xlabel('Motion Strength (% Coh.)')
#     #plt.legend(loc='best')
#     fig.savefig('/home/ramon/Dropbox/Proyectos_Postdoc/Esteki_Kiani/plots/chronometric_monkey_%s_def2.pdf'%monkeys[k],dpi=500,bbox_inches='tight')
...
#     print ('Test Psy ',np.mean(params_psy[:,1,1]-params_psy[:,1,2]),sem(params_psy[:,1,1]-params_psy[:,1,2]),scipy.stats.ttest_rel(params_psy[:,1,1],params_psy[:,1,2]))
#     print ('Test RT ',np.mean(params_rt[:,3,1]-params_rt[:,3,2]),sem(params_rt[:,3,1]-params_rt[:,3,2]),scipy.stats.ttest_rel(params_rt[:,3,1],params_rt[:,3,2]))
...
# print ('Test Psy ALL ',np.nanmean(params_psy_all[:,1,1]-params_psy_all[:,1,2]),sem(params_psy_all[:,1,1]-params_psy_all[:,1,2],nan_policy='omit'),scipy.stats.ttest_rel(params_psy_all[:,1,1],params_psy_all[:,1,2],nan_policy='omit'))
# print ('Test RT ',np.nanmean(params_rt_all[:,3,1]-params_rt_all[:,3,2]),sem(params_rt_all[:,3,1]-params_rt_all[:,3,2],nan_policy='omit'),scipy.stats.ttest_rel(params_rt_all[:,3,1],params_rt_all[:,3,2],nan_policy='omit'))
...
# #bax.set_xticks([-0.5,-0.2,0,0.2,0.5])
# #plt.xticks([-2.54,0,2.54],['-12.8','0','12.8'])
# fig.savefig('/home/ramon/Dropbox/Proyectos_Postdoc/Esteki_Kiani/plots/psychometric_both_def2.pdf',dpi=500,bbox_inches='tight')
...
# #ax=fig.add_subplot(1,1,1)
# #adjust_spines(ax,['left','bottom'])
# bax.plot(xx_fit,fit_chrono_all_m[:,0],color='black',alpha=0.5)
# bax.fill_between(xx_fit,fit_chrono_all_m[:,0]-fit_chrono_all_sem[:,0],fit_chrono_all_m[:,0]+fit_chrono_all_sem[:,0],color='black',alpha=0.5)
# bax.plot(xx_fit,fit_chrono_all_m[:,1],color='green',linewidth=1,alpha=0.5)
# bax.fill_between(xx_fit,fit_chrono_all_m[:,1]-fit_chrono_all_sem[:,1],fit_chrono_all_m[:,1]+fit_chrono_all_sem[:,1],color='green',alpha=0.5)
# bax.plot(xx_fit,fit_chrono_all_m[:,2],color='blue',linewidth=1,alpha=0.5)
# bax.fill_between(xx_fit,fit_chrono_all_m[:,2]-fit_chrono_all_sem[:,2],fit_chrono_all_m[:,2]+fit_chrono_all_sem[:,2],color='blue',alpha=0.5)
# bax.scatter(xx_coh_all,chrono_all_m[:,0],color='black',s=3)
# bax.scatter(xx_coh_all,chrono_all_m[:,1],color='green',s=3)#,alpha=0.5)
# bax.scatter(xx_coh_all,chrono_all_m[:,2],color='blue',s=3)#,alpha=0.5)
# bax.set_ylabel('Reaction Time (ms)')
# bax.set_xlabel('Motion Strength (% Coh.)')
# #plt.legend(loc='best')
# fig.savefig('/home/ramon/Dropbox/Proyectos_Postdoc/Esteki_Kiani/plots/chronometric_both_def2.pdf',dpi=500,bbox_inches='tight')
...
# ax=fig.add_subplot(1,1,1)
# adjust_spines(ax,['left','bottom'])
# ax.plot(np.arange(12),params_psy_all[0:12,1,0],color='black',label='All M1')
# ax.plot(np.arange(12),params_psy_all[0:12,1,1],color='green',label='Left M1')
# ax.plot(np.arange(12),params_psy_all[0:12,1,2],color='blue',label='Right M1')
# ax.plot(np.arange(30),params_psy_all[12:,1,0],color='black',label='All M2',alpha=0.6)
# ax.plot(np.arange(30),params_psy_all[12:,1,1],color='green',label='Left M2',alpha=0.6)
# ax.plot(np.arange(30),params_psy_all[12:,1,2],color='blue',label='Right M2',alpha=0.6)
# ax.plot(np.arange(30),0.5*np.ones(30),color='black',linestyle='--')
# ax.set_ylabel('Bias Psychometric')
# ax.set_xlabel('Learning Epoch')
# #plt.legend(loc='best')
# fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/learning_psycho_bias.pdf',dpi=500,bbox_inches='tight')

# fig=plt.figure(figsize=(3,2))
# ax=fig.add_subplot(1,1,1)
# adjust_spines(ax,['left','bottom'])
# ax.plot(np.arange(12),params_psy_all[0:12,1,2]-params_psy_all[0:12,1,1],color='black')
# ax.plot(np.arange(30),params_psy_all[12:,1,2]-params_psy_all[12:,1,1],color='black',alpha=0.6)
# #ax.plot(np.arange(30),0.5*np.ones(30),color='black',linestyle='--')
# ax.set_ylabel('Bias Psychometric')
# ax.set_xlabel('Learning Epoch')
# ax.set_ylim([0,12])
# #plt.legend(loc='best')
# fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/learning_psycho_bias_diff.pdf',dpi=500,bbox_inches='tight')

# # Plot Chrono Params through Learning
# fig=plt.figure(figsize=(3,2))
# ax=fig.add_subplot(1,1,1)
# adjust_spines(ax,['left','bottom'])
# ax.plot(np.arange(12),params_rt_all[0:12,1,0],color='black',label='All M1')
# ax.plot(np.arange(12),params_rt_all[0:12,1,1],color='green',label='Left M1')
# ax.plot(np.arange(12),params_rt_all[0:12,1,2],color='blue',label='Right M1')
# ax.plot(np.arange(30),params_rt_all[12:,1,0],color='black',label='All M2',alpha=0.6)
# ax.plot(np.arange(30),params_rt_all[12:,1,1],color='green',label='Left M2',alpha=0.6)
# ax.plot(np.arange(30),params_rt_all[12:,1,2],color='blue',label='Right M2',alpha=0.6)
# ax.plot(np.arange(30),0.5*np.ones(30),color='black',linestyle='--')
# ax.set_ylabel('Bias Chronometric')
# ax.set_xlabel('Learning Epoch')
# #plt.legend(loc='best')
# fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/learning_chrono_bias.pdf',dpi=500,bbox_inches='tight')

# fig=plt.figure(figsize=(3,2))
# ax=fig.add_subplot(1,1,1)
# adjust_spines(ax,['left','bottom'])
# ax.plot(np.arange(12),params_rt_all[0:12,1,2]-params_rt_all[0:12,1,1],color='black')
# ax.plot(np.arange(30),params_rt_all[12:,1,2]-params_rt_all[12:,1,1],color='black',alpha=0.6)
# #ax.plot(np.arange(30),0.5*np.ones(30),color='black',linestyle='--')
# ax.set_ylabel('Bias Chronometric')
# ax.set_xlabel('Learning Epoch')
# #ax.set_ylim([0,12])
# #plt.legend(loc='best')
# fig.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/learning_chrono_bias_diff.pdf',dpi=500,bbox_inches='tight')
