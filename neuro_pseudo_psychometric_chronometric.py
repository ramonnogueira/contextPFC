...
for ii in range(n_rand):
    ...
    pseudo=miscellaneous.pseudopop_coherence_context_correct(abs_path,files,talig,dic_time,steps,thres,nt,1,perc_tr,True,tpre_sacc,group_coh,shuff=False)
...
for t_plot in range(steps):
    ...
    ax.set_ylabel('Probability Right Response')
    ax.set_xlabel('Evidence Right Choice (%)')
    plt.xticks(xx[indnan0],coh_plot[0][indnan0])
