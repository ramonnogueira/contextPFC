...
def cv_kmeans(x_tr,x_te,n_clusters):
    perf_k=nan*np.zeros((n_clusters-1,2))
    for i in range(1,n_clusters):
        kmeans=KMeans(n_clusters=i).fit(x_tr.transpose()) # Transpose X because we want to see clusters wrt to pops of neurons
        perf_k[i-1,0]=kmeans.score(x_tr.transpose())
        perf_k[i-1,1]=kmeans.score(x_te.transpose())
    return perf_k
...
for k in range(len(monkeys)):
    ...
    n_neu=np.shape(pseudo['pseudo_all'])[-1]
    
    perf_all=nan*np.zeros((steps,n_rand,2))
    weights_all=nan*np.zeros((steps,n_rand,2,n_neu))
    ...
    for kk in range(steps):
        ...
        mat_cond=nan*np.zeros((n_rand,4,n_neu))
        mat_cond_tr=nan*np.zeros((n_rand,4,n_neu))
        mat_cond_te=nan*np.zeros((n_rand,4,n_neu))
        
        for ii in range(n_rand):
            # Fill the population matrix
            mat_cond[ii,0]=np.mean(pseudo_all[ii][ind00],axis=0)
            mat_cond_tr[ii,0]=np.mean(pseudo_tr[ii][ind00],axis=0)
            mat_cond_te[ii,0]=np.mean(pseudo_te[ii][ind00],axis=0)
            mat_cond[ii,1]=np.mean(pseudo_all[ii][ind01],axis=0)
            mat_cond_tr[ii,1]=np.mean(pseudo_tr[ii][ind01],axis=0)
            mat_cond_te[ii,1]=np.mean(pseudo_te[ii][ind01],axis=0)
            mat_cond[ii,2]=np.mean(pseudo_all[ii][ind10],axis=0)
            mat_cond_tr[ii,2]=np.mean(pseudo_tr[ii][ind10],axis=0)
            mat_cond_te[ii,2]=np.mean(pseudo_te[ii][ind10],axis=0)
            mat_cond[ii,3]=np.mean(pseudo_all[ii][ind11],axis=0)
            mat_cond_tr[ii,3]=np.mean(pseudo_tr[ii][ind11],axis=0)
            mat_cond_te[ii,3]=np.mean(pseudo_te[ii][ind11],axis=0)

            pp=cv_kmeans(mat_cond_tr[ii],mat_cond_te[ii],n_neu)
            plt.plot(pp[:,0],color='blue')
            plt.plot(pp[:,1],color='red')
            plt.show()
            ...
            weights_all[kk,ii,0]=cl.coef_[0]
            # Context
            cl=LogisticRegression(C=1/reg,class_weight='balanced')
            ...
            weights_all[kk,ii,1]=cl.coef_[0]

            
    perf_all_m=np.nanmean(perf_all,axis=1)
    weights_all_m=np.nanmean(weights_all,axis=1)
    n_sh=10000
    for t in range(steps):
        print (t)
        x=abs(weights_all_m[t,0])
        y=abs(weights_all_m[t,1])
