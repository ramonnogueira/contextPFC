...
coh_uq=np.unique(group_coh)
if np.sum(np.isnan(coh_uq))>0:
    coh_uq=coh_uq[0:-1]

disp_vec=np.arange(len(coh_uq)-1)

for k in range(len(monkeys))[0:1]:
    print (monkeys[k])
    abs_path='/home/ramon/Dropbox/Esteki_Kiani/data/sorted/late/%s/'%(monkeys[k]) 
    files=os.listdir(abs_path)
        
    lin_comp_local=nan*np.zeros((len(files),steps,len(disp_vec),len(coh_uq)-1,n_rand,3))
    ccgp_local=nan*np.zeros((len(files),steps,len(disp_vec),len(coh_uq)-1,n_rand,2))
    perf=nan*np.zeros((len(files),len(coh_uq)-1,steps,2))
    for kk in range(len(files)):
        ...
        coherence=beha['coh_resol'][ind_corr] #Cuidado!!
        ...
        features[:,0]=coherence
        features[:,1]=context
        ind_enc=~np.isnan(coherence)
        feat_norm=normalize_feat(features[ind_enc])
        fr_norm_enc=fr_norm[ind_enc]
        ...
        for j in range(steps):
            ...
            g=-1
            for train_index, test_index in cv.split(fr_norm_enc[:,:,j]):
                g=(g+1)
                cl=LinearRegression()
                cl.fit(feat_norm[train_index],fr_norm_enc[:,:,j][train_index])
                y_pred=cl.predict(feat_norm[test_index])
                fr_surr_pre[0,test_index,:,j]=y_pred
                sc=score(y_pred,fr_norm_enc[:,:,j][test_index])
        ...
        for jj in range(0,len(coh_uq)-1):
            for j in range(steps):
                ...
                index=np.where(((coherence==coh_uq[jj])|(coherence==coh_uq[jj+1]))&(rt>min_t))[0]
                try:
                    #perf[kk,jj,j]=class_twovars(fr_norm[index][:,:,j],coherence[index],context[index])
                    perf[kk,jj,j]=class_twovars(fr_surr[index][:,:,j],coherence[index],context[index])
                except:
                    print ('Error in ','Coh ',coh_uq[jj],'step ',j)
                
        print ('Coherence \n',np.nanmean(perf,axis=0)[:,:,0])
        print ('Context \n',np.nanmean(perf,axis=0)[:,:,1])

        ###############################################
        # Tasks and Shifted-CCGP        
        for j in range(steps):
            print (j)
            min_t=(j*dic_time[3]+dic_time[2]+tpre_sacc) # for each time step temporal threshold at which the RT needs to be bigger than that
            for d in range(len(disp_vec)):
                print (' ',d)
                disp=disp_vec[d]
                c_steps=(len(coh_uq)-1-disp)
                for jj in range(c_steps):
                    ind00=np.where((coherence==coh_uq[jj+disp])&(context==ctx_uq[0])&(rt>min_t))[0]
                    ind01=np.where((coherence==coh_uq[jj])&(context==ctx_uq[1])&(rt>min_t))[0]
                    ind10=np.where((coherence==coh_uq[jj+1+disp])&(context==ctx_uq[0])&(rt>min_t))[0]
                    ind11=np.where((coherence==coh_uq[jj+1])&(context==ctx_uq[1])&(rt>min_t))[0]
                    ...
                    n_min=np.min(np.array([len(ind00),len(ind01),len(ind10),len(ind11)]))
                    for ll in range(n_rand):
                        ind00_s=np.random.choice(ind00,n_min,replace=False)
                        ind01_s=np.random.choice(ind01,n_min,replace=False)
                        ind10_s=np.random.choice(ind10,n_min,replace=False)
                        ind11_s=np.random.choice(ind11,n_min,replace=False)
                        ind_all=np.sort(np.concatenate((ind00_s,ind01_s,ind10_s,ind11_s)))
                    
                        feat_bin=feat_bin_pre[ind_all]
                        #ut=fr_norm[ind_all][:,:,j]
                        ut=fr_surr[ind_all][:,:,j]
                        # Lin and XOR tasks
                        try:
                            lin_comp_local[kk,j,d,jj,ll]=lin_comp_task(ut,feat_bin)
                        except:
                            None
                            #print ('step time ',j,'displace ',d,'step coh ',jj,'rand ',ll)
                        # Abstraction
                        try:
                            ccgp_local[kk,j,d,jj,ll]=np.mean(abstraction_2D(ut,feat_bin,reg=1)[:,:,1],axis=1)
                        except:
                            None
        print (np.nanmean(lin_comp_local,axis=(0,4)))
        print (np.nanmean(ccgp_local,axis=(0,4)))
