...
for k in range(len(monkeys)):
    if monkeys[k]=='Niels':
        ...
        ind_uu=np.array([0,1,2,3,5,6,7,8,9,11,12,13,14]) # Niels doesn't have the 4.5 coherence level
    ...
    files=np.array(files_pre[order])#[0] # Only first session!
    print (files)
    ...
    for kk in range(len(files)):
        ...
        print (len(change_ctx),len(ind_ch))
        print (ind_ch)
        ...
        ctx_nan0=nan*np.zeros(len(choice))
        ctx_nan1=nan*np.zeros(len(choice))
        ctx_nan0[context==0]=0.5
        ctx_nan1[context==1]=0.5
    
        #for i in range(len(ind_ch)):
        #    print (context[(ind_ch[i]-10):(ind_ch[i]+10)])
        
        bins=40
        choice_sm=nan*np.zeros(len(choice))
        for i in range(len(change_ctx)):
            choice_sm[i]=np.mean(choice[i:(i+bins)])

        plt.plot(choice_sm,color='black')
        plt.ylabel('Mean Choice')
        plt.xlabel('Trials')
        plt.title('Session %i'%(kk+1))
        plt.plot(ctx_nan0,color='green',linestyle='--',lw=4)
        plt.plot(ctx_nan1,color='blue',linestyle='--',lw=4)
        plt.ylim([0,1])
        plt.savefig('/home/ramon/Dropbox/Esteki_Kiani/plots/bias_trials_%s_s_%i.png'%(monkeys[k],kk+1))
        plt.close('all')
        #plt.show()
