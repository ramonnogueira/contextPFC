    ...
    for kk in range(len(files)):
        ...
        for h in range(len(indch_ct01)): #from left to right            
            for j in range(t_back):
                try:
                    y_matrix01b_neu_pre.append(choice_neu[indch_ct01[h]-t_back+j+1])
                    y_matrix01b_pre.append(choice[indch_ct01[h]-t_back+j+1])
                    x_matrix01b_pre.append([coherence[indch_ct01[h]-t_back+j+1],0])
                except:
                    None
            for j in range(t_forw):
                try:
                    y_matrix01a_neu_pre.append(choice_neu[indch_ct01[h]+j+1])
                    y_matrix01a_pre.append(choice[indch_ct01[h]+j+1])
                    x_matrix01a_pre.append([coherence[indch_ct01[h]+j+1],j+1])
                except:
                    None
        ...
        for h in range(len(indch_ct10)): #from left to right
            for j in range(t_back):
                try:
                    y_matrix10b_neu_pre.append(choice_neu[indch_ct10[h]-t_back+j+1])
                    y_matrix10b_pre.append(choice[indch_ct10[h]-t_back+j+1])
                    x_matrix10b_pre.append([coherence[indch_ct10[h]-t_back+j+1],0])
                except:
                    None
            for j in range(t_forw):
                try:
                    y_matrix10a_neu_pre.append(choice_neu[indch_ct10[h]+j+1])
                    y_matrix10a_pre.append(choice[indch_ct10[h]+j+1])
                    x_matrix10a_pre.append([coherence[indch_ct10[h]+j+1],j+1])
                except:
                    None

    # Behavior 01
    y_matrix01b_neu=np.array(y_matrix01b_neu_pre,dtype=np.int16)
    y_matrix01b=np.array(y_matrix01b_pre,dtype=np.int16)
    x_matrix01b=np.array(x_matrix01b_pre)
    x01b_sm=sm.add_constant(x_matrix01b)
    cl01b=sm.Logit(y_matrix01b,x01b_sm).fit(disp=False)
    y_matrix01a_neu=np.array(y_matrix01a_neu_pre,dtype=np.int16)
    y_matrix01a=np.array(y_matrix01a_pre,dtype=np.int16)
    x_matrix01a=np.array(x_matrix01a_pre)
    x01a_sm=sm.add_constant(x_matrix01a)
    print (x01a_sm,np.shape(x01a_sm))
    cl01a=sm.Logit(y_matrix01a,x01a_sm).fit(disp=False)
    # Neuronal 01
    cl01b_neu=sm.Logit(y_matrix01b_neu,x01b_sm).fit(disp=False)
    cl01a_neu=sm.Logit(y_matrix01a_neu,x01a_sm).fit(disp=False)
    dic['Left to Right b']=cl01b.params
    dic['Left to Right bse b']=cl01b.bse
    dic['Left to Right neu b']=cl01b_neu.params
    dic['Left to Right neu bse b']=cl01b_neu.bse
    dic['Left to Right a']=cl01a.params
    dic['Left to Right bse a']=cl01a.bse
    dic['Left to Right neu a']=cl01a_neu.params
    dic['Left to Right neu bse a']=cl01a_neu.bse

    # Behavior 10
    y_matrix10b_neu=np.array(y_matrix10b_neu_pre,dtype=np.int16)
    y_matrix10b=np.array(y_matrix10b_pre,dtype=np.int16)
    x_matrix10b=np.array(x_matrix10b_pre)
    x10b_sm=sm.add_constant(x_matrix10b)
    cl10b=sm.Logit(y_matrix10b,x10b_sm).fit(disp=False)
    y_matrix10a_neu=np.array(y_matrix10a_neu_pre,dtype=np.int16)
    y_matrix10a=np.array(y_matrix10a_pre,dtype=np.int16)
    x_matrix10a=np.array(x_matrix10a_pre)
    x10a_sm=sm.add_constant(x_matrix10a)
    cl10a=sm.Logit(y_matrix10a,x10a_sm).fit(disp=False)
    # Neuronal 10
    cl10b_neu=sm.Logit(y_matrix10b_neu,x10b_sm).fit(disp=False)
    cl10a_neu=sm.Logit(y_matrix10a_neu,x10a_sm).fit(disp=False)
    dic['Right to Left b']=cl10b.params
    dic['Right to Left bse b']=cl10b.bse
    dic['Right to Left neu b']=cl10b_neu.params
    dic['Right to Left neu bse b']=cl10b_neu.bse
    dic['Right to Left a']=cl10a.params
    dic['Right to Left bse a']=cl10a.bse
    dic['Right to Left neu a']=cl10a_neu.params
    dic['Right to Left neu bse a']=cl10a_neu.bse
...
# Original
# weib=nan*np.zeros((len(monkeys),len(stage_vec),2,2))
# weia=nan*np.zeros((len(monkeys),len(stage_vec),2,2))
# #wei_std=nan*np.zeros((len(monkeys),len(stage_vec),2,2))
# wei_neu=nan*np.zeros((len(monkeys),len(stage_vec),2,2))
# #wei_neu_std=nan*np.zeros((len(monkeys),len(stage_vec),2,2))
for k in range(len(monkeys)):
    ...
    beha_lr=calculate_weights_lr(files,reg,talig,dic_time,t_back,t_forw,thres)
#     # Behavioral
#     weib[k,ss,0]=beha_lr['Left to Right b']
#     weib[k,ss,1]=beha_lr['Right to Left b']
#     weia[k,ss,0]=beha_lr['Left to Right a']
#     weia[k,ss,1]=beha_lr['Right to Left a']
#     # Neuronal
...
# wei_mb_pre=np.mean(weib,axis=0)
# wei_mb0=1/2*(wei_mb_pre[:,1,0]-wei_mb_pre[:,0,0])
# wei_mb1=1/2*(wei_mb_pre[:,0,1]+wei_mb_pre[:,1,1])
# w0cb=(wei_mb0/wei_mb1)

# wei_ma_pre=np.mean(weia,axis=0)
# wei_ma0=1/2*(wei_ma_pre[:,1,0]-wei_ma_pre[:,0,0])
# wei_ma1=1/2*(wei_ma_pre[:,0,1]+wei_ma_pre[:,1,1])
# w0ca=(wei_ma0/wei_ma1) 
